...
//        if (! laggedBehind(task)) { // Not lagged behind.
        ...
        if (task->getParameter(SimpleTask::DEGREEFUL) > 0) {
...
//    cout << "SensorTimeShare:" << endl;
    ...
    for (int i = 0; i < numSensors; i ++) {
        ...
        if (count != 0) {
            perCMWorkload[i] = averageWorkloads[i] / count;
        }
        else {
            perCMWorkload[i] = 0;
        }
    ...
    for (int i = 0; i < numCMs; i ++) {
        ...
        for (int j = 0; j < numSensors; j ++) {
            if (CMSensors[i][j]) {
                total += perCMWorkload[j];
            }
        }
        for (int j = 0; j < numSensors; j ++) {
            if (CMSensors[i][j]) {
                sensorTimeShare[i][j] = perCMWorkload[j] / total;
            }
//            cout << sensorTimeShare[i][j] << " ";
        }
//        cout << endl;
    }
}
